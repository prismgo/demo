...
本工作区使用 Docker Compose 提供 MySQL、PostgreSQL、SQL Server、Redis 和 RabbitMQ，SQLite 由宿主机上的测试进程直接使用。所有基础设施配置位于 `demo/.dev/docker/`，统一入口是项目根目录的 `./demo/dev`。
...
`./demo/dev up` 会启动全部容器、等待健康检查、初始化 SQL Server 测试数据库，并生成 `demo/.dev/runtime/test.env`。重复执行是安全的。

只启动当前任务需要的依赖：

```bash
./demo/dev up mysql redis
./demo/dev up rabbitmq
./demo/dev up sqlite
```
...
| 命令 | 行为 |
|---|---|
| `./demo/dev init` | 创建根目录 Agent 指令链接，把 `demo/.agents/skills/` 下的项目 skills 软链到 `.agents`/`.claude`，克隆缺失仓库，初始化 `demo/.env` 和 `go.work`，并下载框架 Go 依赖 |
| `./demo/dev up [service...]` | 启动全部或指定服务并等待健康 |
| `./demo/dev status` | 查看容器状态、端口、账号和环境变量文件 |
...
1. 只启动任务需要的服务，例如 `./demo/dev up rabbitmq`。
2. 运行 `./demo/dev status`，确认目标容器为 `healthy`。
3. 通过 `./demo/dev test <packages>`，或先 `source demo/.dev/runtime/test.env` 再运行框架测试。
4. 报告实际启动的服务、执行的命令，以及测试通过、失败或跳过的数量。
5. 环境变量缺失导致的 `Skip` 不是集成测试通过；必须确认测试确实执行。
6. 普通清理使用 `./demo/dev down`；只有明确需要干净数据时才使用会删除卷的 `./demo/dev reset`。
...
- 端口被占用：在 `demo/.dev/docker/local.env` 覆盖对应宿主机端口，再运行 `./demo/dev up`。
- 容器未健康：运行 `./demo/dev logs <service>` 查看启动日志。
- 修改初始化账号后仍使用旧账号：初始化变量只对空数据卷生效；确认不需要旧数据后运行 `./demo/dev reset`。
