...
本工作区使用 Docker Compose 提供 MySQL、PostgreSQL、SQL Server、Redis 和 RabbitMQ，SQLite 由宿主机上的测试进程直接使用。所有基础设施配置位于 `.dev/docker/`，统一入口是仓库根目录的 `./dev`。
...
`./dev up` 会启动全部容器、等待健康检查、初始化 SQL Server 测试数据库，并生成 `.dev/runtime/test.env`。重复执行是安全的。
...
| 命令 | 行为 |
|---|---|
| `./dev up [service...]` | 启动全部或指定服务并等待健康 |
| `./dev status` | 查看容器状态、端口、账号和环境变量文件 |
| `./dev logs [service]` | 持续查看全部或指定服务日志 |
| `./dev env` | 重新生成并输出测试环境变量 |
| `./dev test [go-test-args]` | 启动环境并在 `framework/` 中执行测试 |
| `./dev down` | 停止容器，保留数据库和队列数据 |
| `./dev reset` | 删除全部测试数据并重新创建；需要交互确认 |
...
1. 只启动任务需要的服务，例如 `./dev up rabbitmq`。
2. 运行 `./dev status`，确认目标容器为 `healthy`。
3. 通过 `./dev test <packages>`，或先 `source .dev/runtime/test.env` 再运行框架测试。
4. 报告实际启动的服务、执行的命令，以及测试通过、失败或跳过的数量。
5. 环境变量缺失导致的 `Skip` 不是集成测试通过；必须确认测试确实执行。
6. 普通清理使用 `./dev down`；只有明确需要干净数据时才使用会删除卷的 `./dev reset`。
...
- 端口被占用：在 `.dev/docker/local.env` 覆盖对应宿主机端口，再运行 `./dev up`。
- 容器未健康：运行 `./dev logs <service>` 查看启动日志。
- 修改初始化账号后仍使用旧账号：初始化变量只对空数据卷生效；确认不需要旧数据后运行 `./dev reset`。
