...
| `dev/` | PrismGo 本地 Demo、联调入口与本地依赖配置 | 独立仓库 |
| `framework/` | `github.com/prismgo/framework` 的源码、测试与框架变更记录 | 独立仓库 |
| `docs/` | PrismGo 对外文档的中英文内容 | 独立仓库 |
| `dev/.dev/docs/` | 本工作区的 Agent 开发说明 | 属于 `dev/` 仓库 |

## 修改归属

- 框架能力、公开 API、组件实现和框架测试放在 `framework/`。
- 用户文档、指南和 API 使用说明放在 `docs/`。
- `dev/` 中的 Demo 只用于复现、验证和展示框架用法，不在 Demo 中复制或绕过框架实现。
- 任务涉及多个仓库时，分别检查 `dev/`、`framework/`、`docs/` 的状态和差异。

## 本地依赖

`dev/` 模块通过以下替换直接使用同级的工作区源码：
