...
func TestExtensionsIncludeRabbitMQProvider(t *testing.T) {
	exts := Extensions()
	if got, want := len(exts), 4; got != want {
	...
	if got, want := named.Name(), "demo.extension.postgres"; got != want {
	...
	named, ok = exts[3].(pcontract.NamedProvider)
	if !ok {
		t.Fatalf("Extensions()[3] type = %T, want provider.NamedProvider", exts[3])
	}
	if got, want := named.Name(), "prismgo.extension.oss"; got != want {
		t.Fatalf("Extensions()[3].Name() = %q, want %q", got, want)
	}
}
