...
var Entries = []Entry{
	...
	entry("queue", "Failed jobs", "处理失败任务", "Dealing With Failed Jobs", "demo:queue failure --connection=redis", "failure", "TestQueueDemoFailure", LevelIntegration, StatusImplemented, "redis", "rabbitmq"),
	entry("queue", "Failed job commands", "清理失败任务", "Dealing With Failed Jobs", "demo:queue failed-commands", "failed-commands", "TestQueueDemoFailedCommands", LevelScenario, StatusImplemented),
	...
	entry("queue", "Lifecycle events", "生命周期事件", "Lifecycle Events", "demo:queue events --connection=redis", "events", "TestQueueDemoEvents", LevelIntegration, StatusImplemented, "redis", "rabbitmq"),
