...
func TestDemoListCommandShowsFeatureOverview(t *testing.T) {
	...
	for _, expected := range []string{
		...
		"Modules: 29 | Implemented: 666/1432 | Planned: 763 | Manual: 3",
...
func TestDemoListCommandJSONAndFeatureStatusFilter(t *testing.T) {
	...
	if len(result.Features) != 12 || result.Features[0].Feature != "cache" || result.Features[1].Feature != "commands" || result.Features[2].Feature != "config" || result.Features[3].Feature != "console" || result.Features[4].Feature != "container" || result.Features[5].Feature != "event" || result.Features[6].Feature != "exception" || result.Features[7].Feature != "filesystem" || result.Features[8].Feature != "horizon" || result.Features[9].Feature != "http-server" || result.Features[10].Feature != "queue" || result.Features[11].Feature != "translation" {
		t.Fatalf("features = %#v, want cache, commands, config, console, container, event, exception, filesystem, horizon, http-server, queue, and translation implemented", result.Features)
	}
...
func TestDemoShowCommandJSONAndFilters(t *testing.T) {
	...
	if result.Framework != "v0.2.2" || len(result.Entries) != 98 {
		t.Fatalf("detail output framework/entries = %q/%d, want v0.2.2/98", result.Framework, len(result.Entries))
	}
...
func TestDemoShowCommandReportsEmptyFilteredFeature(t *testing.T) {
	...
	input := demoInput{
		...
		options:   map[string]string{"status": "manual"},
	}
	if err := command.Handle(commandContext(command, input, &output)); err != nil {
