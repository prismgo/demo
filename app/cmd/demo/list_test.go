...
func TestDemoListCommandShowsFeatureOverview(t *testing.T) {
	...
	for _, expected := range []string{
		...
		"Modules: 29 | Implemented: 1159/1432 | Planned: 270 | Manual: 3",
...
func TestDemoListCommandJSONAndFeatureStatusFilter(t *testing.T) {
	...
	if len(result.Features) != 20 || result.Features[0].Feature != "cache" || result.Features[1].Feature != "commands" || result.Features[2].Feature != "config" || result.Features[3].Feature != "console" || result.Features[4].Feature != "container" || result.Features[5].Feature != "cookie" || result.Features[6].Feature != "event" || result.Features[7].Feature != "exception" || result.Features[8].Feature != "filesystem" || result.Features[9].Feature != "horizon" || result.Features[10].Feature != "http-server" || result.Features[11].Feature != "lifecycle" || result.Features[12].Feature != "logger" || result.Features[13].Feature != "queue" || result.Features[14].Feature != "ratelimit" || result.Features[15].Feature != "redis" || result.Features[16].Feature != "route" || result.Features[17].Feature != "service-provider" || result.Features[18].Feature != "session" || result.Features[19].Feature != "translation" {
		t.Fatalf("features = %#v, want cache, commands, config, console, container, cookie, event, exception, filesystem, horizon, http-server, lifecycle, logger, queue, ratelimit, redis, route, service-provider, session, and translation implemented", result.Features)
	}
...
func TestDemoShowCommandJSONAndFilters(t *testing.T) {
	...
	if result.Framework != "v0.2.2" || len(result.Entries) != 40 {
		t.Fatalf("detail output framework/entries = %q/%d, want v0.2.2/40", result.Framework, len(result.Entries))
	}
