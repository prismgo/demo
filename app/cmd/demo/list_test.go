...
func TestDemoListCommandShowsFeatureOverview(t *testing.T) {
	...
	for _, expected := range []string{
		...
		"Modules: 29 | Implemented: 1221/1432 | Planned: 208 | Manual: 3",
...
func TestDemoShowCommandJSONAndFilters(t *testing.T) {
	...
	if result.Framework != "v0.2.2" || len(result.Entries) != 39 {
		t.Fatalf("detail output framework/entries = %q/%d, want v0.2.2/39", result.Framework, len(result.Entries))
	}
