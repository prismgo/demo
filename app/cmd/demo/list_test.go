...
func TestDemoListCommandShowsFeatureOverview(t *testing.T) {
	...
	for _, expected := range []string{
		...
		"Modules: 29 | Implemented: 756/1432 | Planned: 673 | Manual: 3",
...
func TestDemoListCommandJSONAndFeatureStatusFilter(t *testing.T) {
	...
	if len(result.Features) != 14 || result.Features[0].Feature != "cache" || result.Features[1].Feature != "commands" || result.Features[2].Feature != "config" || result.Features[3].Feature != "console" || result.Features[4].Feature != "container" || result.Features[5].Feature != "cookie" || result.Features[6].Feature != "event" || result.Features[7].Feature != "exception" || result.Features[8].Feature != "filesystem" || result.Features[9].Feature != "horizon" || result.Features[10].Feature != "http-server" || result.Features[11].Feature != "logger" || result.Features[12].Feature != "queue" || result.Features[13].Feature != "translation" {
		t.Fatalf("features = %#v, want cache, commands, config, console, container, cookie, event, exception, filesystem, horizon, http-server, logger, queue, and translation implemented", result.Features)
	}
