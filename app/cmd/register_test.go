...
func TestCommandFactoriesIncludeDemoCommands(t *testing.T) {
	...
	if len(factories) != 16 {
		t.Fatalf("CommandFactories() returned %d factories, want 16", len(factories))
	}
	wanted := map[string]bool{"demo:list": false, "demo:show": false, "demo:queue": false, "demo:cache": false, "demo:filesystem": false, "demo:event": false, "demo:config": false, "demo:container": false, "demo:cookie": false, "demo:exception": false, "demo:logger": false, "demo:translation": false, "demo:console": false, "demo:commands": false, "demo:http-server": false, "demo:session": false}
