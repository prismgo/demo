...
func TestCommandFactoriesIncludeDemoCommands(t *testing.T) {
	...
	if len(factories) != 6 {
		t.Fatalf("CommandFactories() returned %d factories, want 6", len(factories))
	}
	wanted := map[string]bool{"demo:list": false, "demo:show": false, "demo:queue": false, "demo:cache": false, "demo:translation": false, "demo:commands": false}
